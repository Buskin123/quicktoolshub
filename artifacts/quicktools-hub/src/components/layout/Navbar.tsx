...
export function Navbar() {
  ...
  return (
    <nav className="sticky top-0 z-50 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 shadow-sm transition-colors">
      ...
      {mobileOpen && (
        <div className="md:hidden border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 px-4 py-3 space-y-1 transition-colors">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
            ...
            {tools
              ...
              .map((tool) => (
                <Link
                  key={tool.id}
                  href={tool.path}
                  className="flex items-center gap-2 px-4 py-2 text-sm text-gray-600 dark:text-gray-300 hover:text-primary"
                  onClick={() => setMobileOpen(false)}
                >
                  <tool.icon className="w-4 h-4" />
                  {tool.title}
